    ...
    function NodeManager(envName, nodeId, baseDir, logPath) {
        ...
        me.getEntryPointGroup = function () {
            ...
            for (var i = 0, node; node = nodes[i]; i++) {
                ...
                if (!nodeGroupCache[node.nodeGroup]) {
                    nodeGroupCache[node.nodeGroup] = true;
                }
            ...
            function checkCacheNodeGroup(checkedGroup) {
                for (var nodeGroup in nodeGroupCache) {
                    if (nodeGroup.indexOf(checkedGroup) != -1) {
                        return nodeGroup;
                    }
                }
            }
            ...
            if (!group) {
                if (!nodeGroupCache[CP]) {
                    group = checkCacheNodeGroup(BL) || checkCacheNodeGroup(LB) || checkCacheNodeGroup(CP)
                }
            }

            return { result : 0, group : group || "" };
        };
